...
object ParallelLogicTests {
    ...
    private fun asserter(helper: GameTestHelper): Int.(Int) -> (Unit) {
        var index = 0
        return { expected: Int ->
            helper.assertTrue(
                (this == expected),
                "Assertion #${++index}: Expected: $expected, Actual: $this".trim(),
            )
        }
    }
    ...
    fun `Max Multiplier by Input Item`(helper: GameTestHelper) {
        ...
        val assertEquals = asserter(helper)
    ...
    @JvmStatic
    @GameTest(template = "lcr", setupTicks = DELAY)
    fun `Max Multiplier by NC Input Item`(helper: GameTestHelper) {
        val maxLimit = 4
        val (machine, input, _) = getMultiblock(helper)!!
        val assert = asserter(helper)
    ...
    @JvmStatic
    @GameTest(template = "lcr", setupTicks = DELAY)
    fun `Max Ratio by Input Voltage`(helper: GameTestHelper) {
        helper.succeed()
    }

    @JvmStatic
    @GameTest(template = "lcr", setupTicks = DELAY)
    fun `Limit by Output Item`(helper: GameTestHelper) {
        val maxLimit = 8
    ...
    @JvmStatic
    @GameTest(template = "lcr", setupTicks = DELAY)
    fun `Limit by Output Fluid`(helper: GameTestHelper) {
        val maxLimit = 8
        val (machine, input, output) = getMultiblock(helper)!!
        input.inventory.insertItem(0, ItemStack(Blocks.COBBLESTONE, 16), false)
