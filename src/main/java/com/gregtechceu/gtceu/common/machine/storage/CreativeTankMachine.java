...
public class CreativeTankMachine extends QuantumTankMachine {
    ...
    private InteractionResult updateStored(FluidStack fluid) {
        stored = new FluidStack(fluid.getFluidHolder(), 1000, fluid.getComponentsPatch());
        onFluidChanged();
    ...
    public InteractionResult onUse(BlockState state, Level world, BlockPos pos, Player player, InteractionHand hand,
        ...
        if (hit.getDirection() == getFrontFacing() && !isRemote()) {
            ...
            CustomFluidTank source = new CustomFluidTank(new FluidStack(stored.getFluidHolder(), Integer.MAX_VALUE,
    ...
    private class InfiniteCache extends FluidCache {
        ...
        public @NotNull FluidStack drain(int maxDrain, FluidAction action) {
            if (!stored.isEmpty()) return new FluidStack(stored.getFluidHolder(), mBPerCycle, stored.getComponentsPatch());
        ...
        public @NotNull FluidStack drain(FluidStack resource, FluidAction action) {
            if (!stored.isEmpty() && FluidStack.isSameFluidSameComponents(stored, resource))
                return new FluidStack(resource.getFluidHolder(), mBPerCycle, resource.getComponentsPatch());
            return FluidStack.EMPTY;
        }
