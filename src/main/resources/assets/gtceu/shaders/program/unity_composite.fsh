...
vec4 four_k(vec3 textel, vec2 uv) {
    return 0.25 * (
      texture(DiffuseSampler, uv + textel.xx) //  1  1
    + texture(DiffuseSampler, uv + textel.xy) //  1 -1
    + texture(DiffuseSampler, uv + textel.yx) // -1  1
    + texture(DiffuseSampler, uv + textel.yy) // -1 -1
    );
}
...
void main() {
    ...
    vec4 out_color = texture(DiffuseSampler, texCoord + textel.xx);
    out_color += texture(DiffuseSampler, texCoord + textel.xz) * 2.0;
    out_color += texture(DiffuseSampler, texCoord + textel.xy);
    out_color += texture(DiffuseSampler, texCoord + textel.yz) * 2.0;
    out_color += texture(DiffuseSampler, texCoord) * 4.0;
    out_color += texture(DiffuseSampler, texCoord + textel.zx) * 2.0;
    out_color += texture(DiffuseSampler, texCoord + textel.yy);
    out_color += texture(DiffuseSampler, texCoord + textel.zy) * 2.0;
    out_color += texture(DiffuseSampler, texCoord + textel.yx);

    vec4 highLight = texture(DownTexture, texCoord);
    vec4 bloom = BloomIntensive * vec4(out_color.rgb * 0.8 / 16. + highLight.rgb * 0.8, 1.);
