    ...
    public void tick() {
        ...
        for (MonitorGroup group : monitorGroups) {
            ...
            for (IItemComponent component : componentItem.getComponents()) {
                ...
                PacketDistributor.sendToPlayersTrackingChunk(level, new ChunkPos(getPos()),
    ...
    public Widget createUIWidget() {
        ...
        Consumer<MonitorGroup> openGroupConfig = (group) -> {
            ...
            groupConfig.addWidget(new LabelWidget(0, 5, () -> {
                String currentName = "";
                if (configGroup.getFirst() != null) {
                    currentName = configGroup.getFirst().getName();
                }
        ...
        createGroupButton.setOnPressCallback(click -> {
            ...
            if (!selectedTargets.isEmpty()) {
                rightClickCallbacks.getOrDefault(selectedTargets.getFirst().getPos(), () -> {}).run();
            }
        });
        setTargetButton.setOnPressCallback(click -> {
            ...
            if (selectedTargets.isEmpty()) group.setTarget(null);
                ...
                group.setTarget(selectedTargets.getFirst().getPos());
            ...
            for (int col = 0; col <= leftDist + rightDist; col++) {
                ...
                Consumer<Iterator<IMonitorComponent>> callback = (it) -> {
                    ...
                    if (selectedComponents.contains(component)) {
                        ...
                        if (!selectedTargets.isEmpty() && selectedTargets.getFirst() == component) {
                        ...
                        ColorRectTexture rect = new ColorRectTexture(
                                (selectedTargets.isEmpty() || selectedTargets.getFirst() != component) ? Color.RED :
                                        Color.PINK);
                        textures.setTextures(rect, texture);
                    }
                ...
                Runnable rightClickCallback = () -> {
                    if (!selectedTargets.isEmpty()) {
                        if (selectedTargets.getFirst() == component) {
                            ...
                            rightClickCallbacks.get(selectedTargets.getFirst().getPos()).run();
                        }
                    }
