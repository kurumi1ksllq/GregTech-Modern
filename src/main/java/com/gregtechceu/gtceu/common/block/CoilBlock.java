...
public class CoilBlock extends ActiveBlock {
    ...
    public void appendHoverText(ItemStack stack, @Nullable BlockGetter level, List<Component> tooltip,
        ...
        if (GTUtil.isShiftDown()) {
            ...
            tooltip.add(GTUtil.translatable("block.gtceu.wire_coil.tooltip_heat", coilType.getCoilTemperature()));
            tooltip.add(GTUtil.translatable("block.gtceu.wire_coil.tooltip_smelter"));
            tooltip.add(
                    GTUtil.translatable("block.gtceu.wire_coil.tooltip_parallel_smelter", coilType.getLevel() * 32));
            tooltip.add(GTUtil.translatable("block.gtceu.wire_coil.tooltip_energy_smelter",
                    Math.max(1, (4 * coilType.getLevel() * 32 / (8 * coilType.getEnergyDiscount())))));
            tooltip.add(GTUtil.translatable("block.gtceu.wire_coil.tooltip_pyro"));
            tooltip.add(GTUtil.translatable("block.gtceu.wire_coil.tooltip_speed_pyro",
                    coilTier == 0 ? 75 : 50 * (coilTier + 1)));
            tooltip.add(GTUtil.translatable("block.gtceu.wire_coil.tooltip_cracking"));
            tooltip.add(GTUtil.translatable("block.gtceu.wire_coil.tooltip_energy_cracking", 100 - 10 * coilTier));
        } else {
            tooltip.add(GTUtil.translatable("block.gtceu.wire_coil.tooltip_extended_info"));
        }
    }
