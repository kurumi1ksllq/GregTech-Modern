...
import com.gregtechceu.gtceu.core.mixins.client.BufferBuilderAccessor;
import com.gregtechceu.gtceu.core.mixins.client.DefaultedVertexConsumerAccessor;
...
public final class CopyingVertexConsumer implements VertexConsumer {
    ...
    private static ByteBuffer reallocateDirect(ByteBuffer old, int capacity) {
    ...
    public void endVertex() {
        ...
        if ((this.buffer.capacity() - this.buffer.position()) < VERTEX_SIZE_BYTES) {
            ...
            this.buffer = reallocateDirect(this.buffer, newCapacity);
        }
    ...
    public static CopyingVertexConsumer copyFrom(BufferBuilder builder) {
        CopyingVertexConsumer consumer = new CopyingVertexConsumer();

        if (builder instanceof BufferBuilderAccessor accessor) {
            consumer.buffer = accessor.gtceu$getBuffer();
            consumer.currentVertex = accessor.gtceu$getVertices();
        }
        if (builder instanceof DefaultedVertexConsumerAccessor accessor) {
            consumer.hasFixedColor = accessor.gtceu$isDefaultColorSet();

            int a = accessor.gtceu$getDefaultA();
            int r = accessor.gtceu$getDefaultR();
            int g = accessor.gtceu$getDefaultG();
            int b = accessor.gtceu$getDefaultB();
            consumer.fixedColor = ((a & 0xFF) << 24) | ((b & 0xFF) << 16) | ((g & 0xFF) << 8) | (r & 0xFF);
        }
        return consumer;
    }
