...
public class BasicAisleStrategy extends AisleStrategy {
    ...
    public int @NotNull [] getDefaultAisles(@Nullable Map<String, String> map) {
        ...
        for(int i = 0; i < multiAisles.size(); i++) {
            ...
            int multiRepeats = Mth.clamp(GTStringUtils.parseInt(map.get("multi." + 1)), multi[0], multi[1]);
            for(int j = 0; j < multiRepeats; j++) {
                for(int k = multi[2]; k < multi[3]; k++) {
                    int aisleRepeats = Mth.clamp(GTStringUtils.parseInt(map.get("multi." + i + "." + (k - multi[2]))),
    ...
    protected void finish(int[] dimensions, RelativeDirection[] directions, List<PatternAisle> aisles) {
        ...
        if(sum > aisles.size()) {
            GTCEu.LOGGER.error("multiAisles out of bounds. total of {} aisles but {} aisles in multiAisles", aisles.size(), sum);
            multiAisleError();
        }
    ...
    protected void multiAisleError() {
        GTCEu.LOGGER.error("multiAisles in the pattern, formatted as [minRepeats, maxRepeats, startInclusive, endExclusive, actualRepeats]");
