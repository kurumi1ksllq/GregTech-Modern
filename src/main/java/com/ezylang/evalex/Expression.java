...
public class Expression {
    ...
    private EvaluationValue evaluateSubtree(ASTNode startNode, int depth) throws EvaluationException {
        if (depth > configuration.getMaxRecursionDepth()) {
            throw new EvaluationException(startNode.token(), "Max recursion depth exceeded");
        }

        Token token = startNode.token();
        EvaluationValue result;
        switch (token.type()) {
            case NUMBER_LITERAL:
                result = EvaluationValue.numberOfString(token.value(), configuration.getMathContext());
                break;
            case STRING_LITERAL:
                result = EvaluationValue.stringValue(token.value());
                break;
            ...
            case POSTFIX_OPERATOR:
                result =
                        token
                                .operatorDefinition()
                                .evaluate(
                                        this, token, evaluateSubtree(startNode.parameters().get(0), depth + 1));
                break;
            case INFIX_OPERATOR:
    ...
    private EvaluationValue getVariableOrConstant(Token token) throws EvaluationException {
        EvaluationValue result = constants.get(token.value());
        if (result == null) {
            result = getDataAccessor().getData(token.value());
        }
        if (result == null) {
    ...
    private EvaluationValue evaluateFunction(ASTNode startNode, Token token, int depth)
        ...
        for (int i = 0; i < startNode.parameters().size(); i++) {
            if (token.functionDefinition().isParameterLazy(i)) {
                parameterResults.add(convertValue(startNode.parameters().get(i)));
            } else {
                parameterResults.add(evaluateSubtree(startNode.parameters().get(i), depth + 1));
            }
        }

        EvaluationValue[] parameters = parameterResults.toArray(new EvaluationValue[0]);

        FunctionIfc function = token.functionDefinition();
    ...
    private EvaluationValue evaluateArrayIndex(ASTNode startNode, int depth)
            throws EvaluationException {
        EvaluationValue array = evaluateSubtree(startNode.parameters().get(0), depth + 1);
        EvaluationValue index = evaluateSubtree(startNode.parameters().get(1), depth + 1);
        ...
        if (array.isArrayValue() && index.isNumberValue()) {
            ...
            throw EvaluationException.ofUnsupportedDataTypeInOperation(startNode.token());
        }
    }

    private EvaluationValue evaluateStructureSeparator(ASTNode startNode, int depth)
            throws EvaluationException {
        EvaluationValue structure = evaluateSubtree(startNode.parameters().get(0), depth + 1);
        Token nameToken = startNode.parameters().get(1).token();
    ...
    private EvaluationValue evaluateInfixOperator(ASTNode startNode, Token token, int depth)
        ...
        OperatorIfc op = token.operatorDefinition();
        if (op.isOperandLazy()) {
            left = convertValue(startNode.parameters().get(0));
            right = convertValue(startNode.parameters().get(1));
        } else {
            left = evaluateSubtree(startNode.parameters().get(0), depth + 1);
            right = evaluateSubtree(startNode.parameters().get(1), depth + 1);
        }
    ...
    private List<ASTNode> getAllASTNodesForNode(ASTNode node) {
        ...
        nodes.add(node);
        for (ASTNode child : node.parameters()) {
    ...
    public Set<String> getUsedVariables() throws ParseException {
        ...
        for (ASTNode node : getAllASTNodes()) {
            if (node.token().type() == Token.TokenType.VARIABLE_OR_CONSTANT
                    && !constants.containsKey(node.token().value())) {
