...
### Machine interaction method changes

Macine interactions have been split into two different methods: `onUse` and `onUseWithItem`. All previous tool interaction methods exist, but all interaction methods now take a single `ExtendedUseOnContext context` argument.
...
- `IMachineLife` Override `MetaMachine::onMachinePlaced` and `MetaMachine::onMachineDestroyed`.
- `IMachineModifyDrops` Override `MetaMachine::modifyDrops`.
- `IInteractedMachine` Override `MetaMachine::onUse`, `MetaMachine::onUseWithItem` and `MetaMachine::onLeftClick`.

Interfaces replaced with machine traits:
...
- `IHasBatterySlot` - Use `ItemChargerSlotTrait`
- `IHasCircuitSlot` - Use `IntCircuitSlotTrait`
