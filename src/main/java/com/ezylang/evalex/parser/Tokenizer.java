...
import static com.ezylang.evalex.parser.Token.TokenType.BRACE_CLOSE;
...
public class Tokenizer {
    ...
    private boolean implicitMultiplicationPossible(Token currentToken) {
        ...
        return ((previousToken.type() == BRACE_CLOSE && currentToken.type() == BRACE_OPEN)
                || (previousToken.type() == NUMBER_LITERAL
                && currentToken.type() == VARIABLE_OR_CONSTANT)
                || (previousToken.type() == NUMBER_LITERAL && currentToken.type() == FUNCTION)
                || (previousToken.type() == NUMBER_LITERAL && currentToken.type() == BRACE_OPEN));
    }
    ...
    private void isInfixInsteadOfPostfix(Token currentToken) {
        ...
        Token previousToken = getPreviousToken();
        if (previousToken == null || previousToken.type() != POSTFIX_OPERATOR) return;
        String opString = previousToken.value();
        if (operatorDictionary.hasInfixOperator(opString)) {
            OperatorIfc op = operatorDictionary.getInfixOperator(opString);
            setPreviousToken(new Token(previousToken.startPosition(), opString, INFIX_OPERATOR, op));
        }
    }

    private void validateToken(Token currentToken) throws ParseException {

        if (currentToken.type() == STRUCTURE_SEPARATOR && getPreviousToken() == null) {
            throw new ParseException(currentToken, "Misplaced structure operator");
        }

        Token previousToken = getPreviousToken();
        if (previousToken != null
                && previousToken.type() == INFIX_OPERATOR
